...
import { MUSIC_THEME_SOURCE, SFX_GAIN, SFX_SOURCES, SPIN_SOURCE, type SfxId } from './sfx';

/**
 * Imperative audio singleton -- not a hook, because sound has to fire from
 * plain press callbacks, from the slot machine's timer-driven spin sequence
 * (`use-slot-machine.ts`) and from the player store's own mutations, none of
 * which can call one.
 *
 ...
 *   buffer, so a sound can overlap itself without limit and nothing cuts its
 *   own tail off -- the reel-stop ticks land less than 150 ms apart. A media
 *   player (`expo-audio`, AVPlayer/ExoPlayer underneath) is the wrong tool
 *   for this: its first `play()` on a fresh instance carries a start-up cost
 *   that would swallow the opening ticks of a spin.
 *
 * - **Music** stays on `expo-audio`. Decoded to PCM, even this short a loop
 *   would sit resident in memory for nothing, where a streaming media player
 *   costs nothing.
 *
...
/** Context time the spin's fade-out lands at, so `setSfxVolume` doesn't stomp a fade in flight. */
let spinFadeUntil = 0;

let musicPlayer: AudioPlayer | null = null;
/**
 * Whether `startMusic()` has been called this session -- separate from the
 * player's own `playing`, which also goes false while backgrounded or muted,
 * neither of which should count as "stopped". Set the instant `startMusic()`
 * is called even if `initAudio()` hasn't resolved yet (the loading screen's
 * `onDone` and the native setup race on launch); `initAudio` checks this flag
 * once its player exists, so that race can't drop the request and leave the
 * theme silent for the rest of the session.
 */
let musicStarted = false;
let appActive = true;

const SPIN_FADE_MS = 150;

/**
 * What "music on" actually means at the output.
 *
 * The loop is mastered like a track, not like a game bed -- it comes off the
 * encoder at about -11.5 dB loudness, which is louder than most of the
 * effects and would make the UI clicks inaudible underneath it. This drops
 * it to roughly -24.5 dB, which is where the ladder of targets in `sfx.ts`'s
 * `SFX_GAIN` is anchored; the quietest cue still clears it by a couple of dB
 * and the loudest by well over ten.
 *
 ...
 * on native -- on web the module id means nothing to it, so resolve it to
...
export async function initAudio(): Promise<void> {
  ...
  try {
    ...
    // plugin, which would add one along with an Android foreground service,
    // is deliberately not installed (see app.json) -- so the music still
    // stops when the app goes away.
    ...
    // The spin rattle gets its own bus purely so its fade-out can be
    // scheduled without touching the level everything else is playing at.
    const spin = context.createGain();
    spin.gain.value = sfxVolume;
    spin.connect(context.destination);
    ...
    spinBuffer = await context.decodeAudioData(decodeSource(SPIN_SOURCE));
...
export type SfxOptions = {
  ...
  gain?: number;
  /** Playback rate; also shifts pitch. Defaults to 1. */
  rate?: number;
};
...
export function startSpinSound(): void {
  ...
  try {
    ...
    spinGain.gain.setValueAtTime(sfxVolume, now);
...
/**
 * Fades the spin clip out over `SPIN_FADE_MS` rather than cutting it off
 * mid-cycle, which reads as a click -- the result lands mid-rattle more often
 * than not.
 */
export function stopSpinSound(): void {
  if (!initialized || !ctx || !spinGain) return;
  try {
    stopSpinSource(SPIN_FADE_MS / 1000);
  } catch {
    // ignore
  }
}

function stopSpinSource(fadeSeconds: number): void {
  if (!ctx || !spinGain || !spinSource) return;
  ...
  if (fadeSeconds > 0) {
    ...
    spinGain.gain.linearRampToValueAtTime(0, now + fadeSeconds);
    spinFadeUntil = now + fadeSeconds;
  }
...
export function startMusic(): void {
  ...
  // called from the loading screen's `onDone` -- the one place that's
...
export function setSfxVolume(volume: number): void {
  ...
  try {
    ...
    // spin bus alone until its fade-out has landed.
    if (spinGain && ctx && ctx.currentTime >= spinFadeUntil) {
      spinGain.gain.cancelScheduledValues(ctx.currentTime);
      spinGain.gain.setValueAtTime(sfxVolume, ctx.currentTime);
    }
  } catch {
    // ignore
  }
